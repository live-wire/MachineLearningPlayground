...
def gradient_calculator(sensors, x, d):
    ...
    for i, sensor in enumerate(sensors):
        sx += (2 * (x[0] - sensor[0]) * (((sensor[0] - x[0]) ** 2 + (sensor[1] - x[1]) ** 2) ** .5 - d[i])) / (
        ((sensor[0] - x[0]) ** 2 + (sensor[1] - x[1]) ** 2) ** .5)
        sy += (2 * (x[1] - sensor[1]) * (((sensor[0] - x[0]) ** 2 + (sensor[1] - x[1]) ** 2) ** .5 - d[i])) / (
        ((sensor[0] - x[0]) ** 2 + (sensor[1] - x[1]) ** 2) ** .5)
...
############ gradient steps uncomment for part b START
...
# plt.legend()
# plt.show()
############ gradient steps uncomment for part b END
